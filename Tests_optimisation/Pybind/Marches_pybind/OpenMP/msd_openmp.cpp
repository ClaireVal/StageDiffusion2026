...
#include <omp.h>  // Inclusion de OpenMP

namespace py = pybind11;

// Fonction pour traiter la boucle en i (parallélisée avec OpenMP)
void process_i_loop(
    ...
    // Parallélisation de la boucle en i avec OpenMP
    ...
    for (size_t i = 0; i < Nt - 1; ++i) {
        ...
        // Boucle en j (séquentielle pour chaque i)
        for (size_t j = i + 1; j < Nt; ++j) {
            ...
            if (k >= 0 && k < nbBins) {
                // Utilisation de #pragma omp atomic pour éviter les race conditions
                #pragma omp atomic
                sums[k] += dx;
                #pragma omp atomic
                counts[k] += 1;
...
py::tuple MSDLongueVar_numba_opt(
    ...
    // Appel à la fonction pour la boucle en i (parallélisée)
...
PYBIND11_MODULE(msd_module_openmp, m) {
