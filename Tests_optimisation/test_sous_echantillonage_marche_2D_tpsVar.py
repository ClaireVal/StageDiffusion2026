...
@njit(parallel=True)
#Fonction calculant le MSD mais en sous-échantillonant les positions de départ des intervalles de temps dt où dx² est calculé
def MSDLongueVar_ssEch(x,t,n_i):
    Nt = x.shape[0]
    nbBins = 100
    ...
    #On tire aléatoirement n_i indices i
    for i in np.random.randint(0, Nt + 1, size=n_i):
        xi = x[i]
        ti = t[i]
...
Ni_liste = [int(sqrt(N)), N//100, N//10, N//5, N]
...
for Ni in Ni_liste:
	for a in alpha:
	    for cD in cDef:
    		...
    		### Calcul du MSD ###
    		centresMSD, MSDLogVar, stdLogVar = MSDLongueVar_ssEch(pos_reelle, temps, Ni)  
    		...
    		fig.savefig(f'D_f_Ni_{Ni}_swap_{swap}_tpsVar_{tpsVar}_N_{N}_cD_{cD}_alpha_{a}_sigma_{sigma}_eps_{eps}.png', dpi=300, bbox_inches='tight', pad_inches=0.1)   # save the figure to file
    		plt.close(fig)    
    		...
    		fig.savefig(f"Dérivée_Ni_{Ni}_swap_MSD_Méthode_Tube_tolerance_{eps}_nbDt_{nbDT}_N_{N}_cD_{cD}_alpha_{a}_sigma_{sigma}.png", dpi=300, bbox_inches='tight', pad_inches=0.1)        
    		plt.close(fig)
