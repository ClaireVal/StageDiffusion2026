...
N=100
#Définition des paramètres de la loi normale visée
mLoi=1
sigmaLoi=3

### On calcule N variables aléatoires normales de paramètres mLoi et sigmaLoi
y = np.zeros(N)
for i in range(N):
    y[i]= rd.normal(mLoi, sigmaLoi)
...
def exponentielle(x, a):
    return stats.expon.cdf(x, scale=a)
...
if nFonc==1:
    ...
    print("Loi uniforme. Paramètres fittés:", aFit, "et", bFit, "et paramètres de la loi (aLoi,bLoi):", mLoi, "et", sigmaLoi)
if nFonc==2:
    ax.plot(x, normale(x, *popt), label="Courbe fittée", color="red")
    print("Loi normale. Paramètres fittés:", popt[0], "et", popt[1], "et paramètres de la loi (mLoi, sigmaLoi):", mLoi, " et ", sigmaLoi)
if nFonc==3:
    ax.plot(x, exponentielle(x, *popt), label="Courbe fittée", color="red")
    print("Loi exponentielle. Paramètre fitté:", popt[0], "et paramètre de la loi (aLoi):", mLoi)
plt.legend()
