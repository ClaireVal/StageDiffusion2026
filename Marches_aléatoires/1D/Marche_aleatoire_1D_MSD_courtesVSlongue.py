...
def MSDLongue(x,dt):
    n = N - dt                         #nombre de (t1,t2) tels que t2-t1=dt
    sum1 = 0
    for i in range(n):
        sum1 += (x[i+dt]-x[i])**2
    return sum1/n

# Second cas : Moyenne sur M trajectoires

def MSDCourtes(x, dt, M):
    sum1 = 0
    for i in range(M):                      #on parcourt les M trajectoires
        sum1 += (x[i][dt]-x[i][0])**2       #on considère les x de la k-ième trajectoire
    return sum1/M
...
# np.savez("MSD.npz", dtLinListe_L = dtLinListe_L, dtLinListe_C=dtLinListe_C, dtLogListe_L = dtLogListe_L, dtLogListe_C=dtLogListe_C, MSDLin_Long=MSDLin_Long, MSDLin_Cour=MSDLin_Cour, stdLinLong=stdLinLong, stdLinCour=stdLinCour, MSDLog_Long=MSDLog_Long, MSDLog_Cour=MSDLog_Cour, stdLogLong=stdLogLong, stdLogCour=stdLogCour)
...
poptL1, _= curve_fit(lineaire, dtLinListe, MSDLin_Long)
poptC1, _ = curve_fit(lineaire, dtLinListe, MSDLin_Cour)
poptL2, _ = curve_fit(lineaire, dtLogListe, MSDLog_Long)
poptC2, _ = curve_fit(lineaire, dtLogListe, MSDLog_Cour)

print("D(long)=", poptL1[0]/2)
print("D(cour)=", poptC1[0]/2)


D_Log_L = exp(poptL2[0])/2
D_Log_C = exp(poptC2[0])/2

# fig, ax = plt.subplots()             
# ax.plot(dtLinListe, lineaire(dtLinListe, *poptL1), ls="--", color="dodgerblue", label = "MSD long")
# ax.plot(dtLinListe, MSDLin_Long, 'o', color="navy")
# ax.plot(dtLinListe, lineaire(dtLinListe, *poptC1), ls="--", color="salmon", label= "MSD court")
# ax.plot(dtLinListe, MSDLin_Cour, '^', color="orangered")
# ax.set_title(f"Marche aléatoire selon une loi de Bernoulli de paramètre p={p} = positions trajectoire longue \n Dlong=poptL1[0]/2 et Dcourt=poptC1[0]/2", fontsize=16)
# ax.set_xlabel("Position", fontsize=22)
# ax.set_ylabel("Temps", fontsize=22)
...
aL, sigmaaL, bL, sigmabL = fit_lineaire(dtLinListe, MSDLin_Long)
aC, sigmaaC, bC, sigmabC = fit_lineaire(dtLinListe, MSDLin_Cour)
print("Fit linéaire sur la trajectoire longue : a", aL, "sigmaa", sigmaaL, "b", bL, "sigmab", sigmabL)
print("Fit linéaire sur les trajectoires courtes : a", aC, "sigmaa", sigmaaC, "b", bC, "sigmab", sigmabC)

fig, ax = plt.subplots()             
ax.plot(dtLogListe, lineaire(dtLogListe, *poptL2), ls="--", color="dodgerblue", label="Fit trajectoire longue")
ax.plot(dtLogListe, MSDLog_Long, 'o', color="navy", label="Trajectoire longue")
ax.plot(dtLogListe, lineaire(dtLogListe, *poptC2), ls="--", color="salmon", label="Fit trajectoires courtes")
ax.plot(dtLogListe, MSDLog_Cour, '^', color="orangered", label="Trajectoires courtes")
