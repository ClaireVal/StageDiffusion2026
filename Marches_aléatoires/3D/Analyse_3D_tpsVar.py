...
cDef = [0, 0.0001, 0.001, 0.01, 0.1, 0.5, 0.593]
freqAt = nu_0 * exp((-G_M/(kB*T))*10**(-19))
alpha = [0.001, 0.01, 0.1, 1, 10, 100, 1000]


@njit(parallel=True)
def MSDLongueVar_numba_opt(x, t):
    Nt = x.shape[0]
    nbBins = 100
    ...
    for i in prange(Nt - 1):
        ...
        for j in range(i + 1, Nt):
            ...
            if 0 <= k < nbBins:
                # attention : race condition possible → voir note plus bas
                sums[k] += dx
                counts[k] += 1
...
for a in alpha:
    for cD in cDef:
        data = np.load(f"Traj_3D_tpsVariable_N_{N}_cD_{cD}_alpha_{a}.npz", allow_pickle=True)
        pos_reelle=data["pos_reelle"]
        positions=data["positions"]
        temps=data["temps"]
        # temps=np.array([i for i in range(N+1)])   pour voir l'équivalent à temps constant
        Densite=data["Densite"]
        listeDefauts=data["listeDefauts"]
        defautsFin=data["defautsFin"]
        defautsFin = defautsFin.item()  # Extrait le set du 0-d array
        defautsFin = list(defautsFin) 

        ### Calcul du MSD ###
        centresMSD, MSDLogVar, stdLogVar = MSDLongueVar_numba_opt(pos_reelle, temps)     
        ...
        for ind in range(len(centresMSD)-tau//2):
            fLog_Long[ind] = np.mean(np.array([f(MSDLogVar[k],centresMSD[k]) for k in range(ind-tau//2, ind+tau//2, 1)]))
        ...
        print(f"Coefficient de diffusion obtenu pour cDef={cD} et alpha={a}: {D} pour un tube commencé à l'indice {debD}")
        ...
        ax[0].set_xscale("log")
        ax[0].set_yscale("log")      
        ax[0].hlines(D, centresMSD.min(),  centresMSD.max())
        ...
        fig.savefig(f'D_f3_swap_tps_var_N_{N}_cD_{cD}_alpha_{a}_sigma_{sigma}.png', dpi=300, bbox_inches='tight', pad_inches=0.1)   # save the figure to file
        plt.close(fig)    
        ...
        fig.savefig(f"Dérivée3_swap_MSD_Méthode_Tube_tolerance_{eps}_nbDt_{nbDT}_N_{N}_cD_{cD}_alpha_{a}_sigma_{sigma}.png", dpi=300, bbox_inches='tight', pad_inches=0.1)        
        plt.close(fig)
        ...
        fig.savefig(f'Marche3_swap_tps_var_N_{N}_cD_{cD}_alpha_{a}_sigma_{sigma}.png', dpi=300, bbox_inches='tight', pad_inches=0.1)   # save the figure to file
        plt.close(fig)   
        ...
        plt.plot(XDefCourant, YDefCourant, "^", color="firebrick", label='Positions finales défauts', alpha=0.7)
        plt.plot(XDisplay, YDisplay, "x", markersize=2, color="dodgerblue", label='Positions initiales défauts',alpha=0.7)
        plt.legend(prop={'size':11, "loc":'bottom left'})
        fig.savefig(f'Densite3_swap_tps_var_N_{N}_cD_{cD}_alpha_{a}_sigma_{sigma}.png', dpi=300, bbox_inches='tight', pad_inches=0.1)
        plt.close(fig)   
