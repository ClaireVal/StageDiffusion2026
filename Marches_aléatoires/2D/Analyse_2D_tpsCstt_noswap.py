...
for a in alpha:
    for cD in cDef:
        data = np.load(f"Traj_swap_{swap}_tpsVariable_{tpsVar}_N_{N}_cD_{cD}_alpha_{a}.npz", allow_pickle=True)
        ...
        fbisLog_Long = np.zeros(len(centresMSD))
        ...
        for ind in range(len(centresMSD)-tau//2):
            ...
            fbisLog_Long[ind] = np.mean(np.array([fbis(MSDLogVar[k],centresMSD[k]) for k in range(ind-tau//2, ind+tau//2, 1)]))
        ...
        _, fbisCor, debf = AsymptoteTube(fbisLog_Long, eps)
        print(f"Coefficient de diffusion obtenu pour cDef={cD} et alpha={a}: {D}")
        print(f"Facteur de corrélation obtenu pour cDef={cD} et alpha={a}: {fCor}")
        print(f"Facteur de corrélation BIIIIIS obtenu pour cDef={cD} et alpha={a}: {fbisCor} pour un tube commencé à l'indice {debf}")
        ...
        fig.savefig(f'D_f_swap_{swap}_tpsVar_{tpsVar}_N_{N}_cD_{cD}_alpha_{a}_sigma_{sigma}_eps_{eps}.png', dpi=300, bbox_inches='tight', pad_inches=0.1)   # save the figure to file
        plt.close(fig)    
        ...
        fig.savefig(f"Dérivée_swap_MSD_Méthode_Tube_tolerance_{eps}_nbDt_{nbDT}_N_{N}_cD_{cD}_alpha_{a}_sigma_{sigma}.png", dpi=300, bbox_inches='tight', pad_inches=0.1)        
        plt.close(fig)
        ...
        fig.savefig(f'Marche_swap_tps_var_N_{N}_cD_{cD}_alpha_{a}_sigma_{sigma}_eps_{eps}.png', dpi=300, bbox_inches='tight', pad_inches=0.1)   # save the figure to file
        plt.close(fig)   
        ...
        fig.savefig(f'Densite_swap_tps_var_N_{N}_cD_{cD}_alpha_{a}_.png', dpi=300, bbox_inches='tight', pad_inches=0.1)
        plt.close(fig)   
