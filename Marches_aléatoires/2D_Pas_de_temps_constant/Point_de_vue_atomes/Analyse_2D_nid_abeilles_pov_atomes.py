...
n_atomes_analyse = 16  # Nombre d'atomes sur lesquels moyenner le MSD
...
def MSDLongueVar_numba_opt_atomes(x_atomes, t):
   ...
   ###### MSD RANDOM ########
       ...
       for a in range(n_atomes):
           ...
           for j in range(Nt - 1):
               ...
               if t_cum + dt > t_bin:
                   break
               for d in range(2):
                   dx = x_atomes[a, j+1, d] - x_atomes[a, j, d]
                   s += dx * dx
               t_cum += dt
    ...
    for a in prange(n_atomes):
        x = x_atomes[a]           # (Nt, 2) — vue sur l'atome a

        # Accumulateurs locaux à chaque thread (réduit les race conditions)
        ...
        # Réduction dans les tableaux globaux
...
for a in alpha:
    for cD in cDef:
        ...
        for ind in range(len(centresMSD) - tau//2):
            fLog_Long[ind] = np.mean(np.array([f(MSDLogVar[k], centresMSD[k], msdRand[k]) for k in range(ind - tau//2, ind + tau//2, 1)]))
        ...
        fig.savefig(f'D_f_hexa_swap_{swap}_tpsVar_{tpsVar}_N_{N}_cD_{cD}_alpha_{a}_sigma_{sigma}_eps_{eps}.png', dpi=300, bbox_inches='tight', pad_inches=0.1)   # save the figure to file
        plt.close(fig)    
        ...
        fig.savefig(f"Dérivée_hexa_swap_MSD_Méthode_Tube_tolerance_{eps}_nbDt_{nbDT}_N_{N}_cD_{cD}_alpha_{a}_sigma_{sigma}.png", dpi=300, bbox_inches='tight', pad_inches=0.1)        
        plt.close(fig)
        ...
        fig.savefig(f'Marche_hexa_swap_tps_var_N_{N}_cD_{cD}_alpha_{a}_sigma_{sigma}_eps_{eps}.png', dpi=300, bbox_inches='tight', pad_inches=0.1)   # save the figure to file
        plt.close(fig)   
